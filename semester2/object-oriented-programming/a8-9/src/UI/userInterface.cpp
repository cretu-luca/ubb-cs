...
void UserInterface::addUI(){
    ...
    numberOfLikes = getIntegerInput();
    cout << "Release year:";
    releaseYear = getIntegerInput();

    auto *newMovie = new Movie(title, genre, trailerLink, releaseYear, numberOfLikes, false);
    try {
        this->controller->addMovieController(newMovie);
...
void UserInterface::removeByNameUI() {
    ...
    try {
        this->controller->removeMovieByNameController(title);
    } catch (const CustomException& e) {
        cout << e.what();
    }
}
...
void UserInterface::displayMoviesByGenre() {
    ...
    for(int i = 0; i < movieVector.size(); i++) {
        ...
        while(running) {
            ...
            //if (option == 'T')
            //    ShellExecute(0, 0, LPCWSTR(movieVector[i]->getTrailerLink().c_str()), 0, 0, SW_SHOW);
            if(option == 'W') {
