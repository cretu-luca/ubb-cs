...
bool SortedBag::remove(TComp e) {
    ...
    while (current != -1 && info[current] != e) {
        parent = current;
        if (relation(e, info[current])) {
            current = left[current];
            side = -1;
    ...
    if (left[current] == -1 && right[current] == -1) {
        ...
        info[current] = info[successor];

        if (side == -1) {
            left[currentParent] = right[successor];
        } else {
            right[currentParent] = right[successor];
        }

        if (right[successor] != -1) {
    ...
    this->nextEmpty[current] = firstEmpty;
...
int SortedBag::nrOccurrences(TComp e) const {
    return nrOccurrencesRecursive(root, e);
}
...
int SortedBag::nrOccurrencesRecursive(int node, TComp e) const {
    ...
    return (info[node] == e) + nrOccurrencesRecursive(left[node], e) + nrOccurrencesRecursive(right[node], e);
}
