...
public:
    ...
    void TSPDFS(int node, vector<int>& visited, vector<int>& path);
    void TSP_backtracking();
    void backtracking(int node, int length, vector<int>& visited, int& minCost, int& currentCost, vector<int>& path, vector<int>& minPath);
    bool validHamiltonianGraph(vector<int>& visited) const;
