...
int main() {
    ...
    while(1) { 
        ...
        int size = scanf("%s", tcp_buffer);

        int bytes_sent = send(tcp_sock, tcp_buffer, strlen(tcp_buffer), 0);

        int recv_len = recvfrom(udp_sock, buffer, BUFFER_SIZE, 0, (struct sockaddr*) &udp_addr, &addr_len);
        buffer[recv_len] = '\0';
