...
int main() {
    ...
    char send_buffer[BUFFER_SIZE] = "cristi", recv_buffer[BUFFER_SIZE], server_result[BUFFER_SIZE], garbage[BUFFER_SIZE]; 
    ...
    while(1 && trolls < 3) {
        ...
        if(op == 0) { operator = '+'; result = num1 + num2; }
        if(op == 1) { operator = '-'; result = num1 - num2; }
        if(op == 2) { operator = '*'; result = num1 * num2; }
        if(op == 3) { operator = '/'; result = num1 / num2; }

        int messi = snprintf(send_buffer, sizeof(send_buffer), "%d%c%d", num1, operator, num2);
